    ...
    <div class="content">
      ...
      <div class="matrixs">
          ...
          <div v-if="isMatrix" class="matrix">
            ...
            <div class="table">
              <table>
                ...
                <tr>
                  <th></th>
                  <th v-for="i in  matrix_incident[0].length">{{ i }}</th>
                </tr>
                <tr v-for="j in matrix_incident.length">
                  <td style="font-weight: bold; color: white">{{ j }}</td>
                  <td style="background-color: white" v-for="i in  matrix_incident[0].length">
                    {{ matrix_incident[j - 1][i - 1] }}
                  </td>
                </tr>
              </table>
            </div>
        ...
        <div v-if="isMatrix" class="left-incident">
          ...
          <ul>
          <li v-for="[key, value] of Object.entries(left_matrix_incident)">{{ `G(${key})`}}<sup>-</sup> = {{`(${value})` }}</li>
          </ul>
        </div>
...
export default {
  ...
  methods: {
    Result() {
      try{
        if(this.sizeMatrix===null){
          this.error="Введите размер матрицы!"
          return;
        }
